...
setup_advanced.py
-----------------
Provisions all AWS resources required for CAI_04/advanced:

  1. IAM Lambda execution role (bedrock:InvokeModel, s3 read/write, logs)
  2. Lambda function: prompt-pipeline-handler
  3. S3 event notification: prompt_inputs/*.json → Lambda
  4. Uploads prompt templates to S3 so Lambda can read them
  5. CI IAM user for GitHub Actions (s3:PutObject)
...
LAMBDA_ROLE_NAME = "PromptPipelineLambdaRole"
FUNCTION_NAME    = "prompt-pipeline-handler"
CI_USERNAME      = "prompt-pipeline-advanced-ci-user"
CI_POLICY_NAME   = "PromptPipelineAdvancedCIPolicy"
...
def get_or_create_role() -> str:
    ...
    policy = {
        ...
        "Statement": [
            ...
            {"Effect": "Allow", "Action": "bedrock:InvokeModel", "Resource": MODEL_ARN},
        ],
    }
    iam.put_role_policy(RoleName=LAMBDA_ROLE_NAME, PolicyName="PromptPipelineLambdaPolicy", PolicyDocument=json.dumps(policy))
...
def configure_s3_notification(fn_arn: str):
    try:
        lam.add_permission(
            FunctionName=FUNCTION_NAME, StatementId="s3-invoke-prompt",
    ...
    print(f"Configured S3 trigger: prompt_inputs/*.json → {FUNCTION_NAME}")


def upload_templates():
    if not TEMPLATES_DIR.exists():
        print(f"  No templates dir found at {TEMPLATES_DIR}, skipping upload.")
        return
    for f in TEMPLATES_DIR.glob("*.txt"):
        key = f"prompt_templates/{f.name}"
        s3.upload_file(str(f), S3_BUCKET, key, ExtraArgs={"ContentType": "text/plain"})
        print(f"  Uploaded template: s3://{S3_BUCKET}/{key}")
...
if __name__ == "__main__":
    ...
    print("\nUploading prompt templates to S3...")
    upload_templates()
    ...
    print(f"  S3_BUCKET_BETA        = {S3_BUCKET}")
